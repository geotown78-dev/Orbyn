...
export const ServerInviteCard = ({ serverId }: Props) => {
  ...
  useEffect(() => {
    const fetchServerData = async () => {
      ...
      if (server) {
        ...
        const { count: memberCount } = await supabase.from('server_members').select('*', { count: 'exact', head: true }).eq('server_id', serverId);
        setMembersCount(memberCount || 0);
        
        // Let's just mock online count for now, or fetch all users and check online status if we had that
        // Since we don't have online status in server_members, let's just make it a random number or similar to membersCount
        setOnlineCount(Math.max(1, Math.floor((memberCount || 0) * 0.4)));
      }
    };
    fetchServerData();
  }, [serverId]);
